...
def fetch_posters(movie_id):
    url = f'https://api.themoviedb.org/3/movie/{movie_id}?api_key=1f6d523ea6edc40fee23edd18c28f1d4&language=en-US'
    ...
    for attempt in range(retries):
        try:
            ...
            # Raise an HTTPError for other bad status codes
            ...
            time.sleep(backoff)  # Wait before next retry
...
def get_details(selected_movie_name):
    ...
    base_path = "Files"
    pickle_file_path = os.path.join(base_path, "movies_dict.pkl")
    with open(pickle_file_path, 'rb') as pickle_file:
        loaded_dict = pickle.load(pickle_file)

    movies = pd.DataFrame.from_dict(loaded_dict)

    pickle_file_path = os.path.join(base_path, "movies2_dict.pkl")
    with open(pickle_file_path, 'rb') as pickle_file:
    ...
    if a.empty:
        ...
        raise ValueError(f"No details found for movie: {selected_movie_name}")
